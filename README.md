...
Để truyền config cho một application:

- Enviroment variables
- File config
- Commandline arguments.

Để sử dụng env variables trực tiếp trong pods khai báo trong YAML:

```yaml
      env:
        - name:
          value: # Số phải để trong ""
         
```

Để override main proces và tham số truyền vào khi container start:
...
Thay vì khai báo fixed config trong các container làm cho việc tái sử dụng pod yaml ở nhiều môi trường với các config khác nhau không còn. Sử dụng configMaps cho các insensitive config.

### ConfigMaps

Là một resource cho phép lưu trữ và quản lí các application config, thường là các insensitive data. Có scoped là namespace-scoped, vì vây chỉ có thể được sử dụng từ các resource khác trong cùng namespace. 

Để tạo configMap sử dụng ku create configmaps hoặc sử dụng yaml file.

```yaml
kubectl create configmaps --from-literal=key=value --from-file=[directoryPath] hoặc --from-file=Key=File-path 
# Với các file, mặc định key sẽ là tên file, value là nội dung của file
...
Để sử dụng key value pairs trong ConfigMaps sử dụng [pod.spec.containers.env.valueFrom.configMapKeyRef.name](http://pod.spec.containers.env.valueFrom.configMapKeyRef.name) và key tương ứng cho trường hợp chỉ định từng key cụ thể. 

Để lấy toàn bộ key value paris trong ConfigMaps sử dụng pod.spec.containers.envFrom.
 ...
 - image: some-image
	 ...
	 - prefix: CONFIG_ 
		 configMapRef: 
			 name: my-config-map 
	 env:
	 - name: # tên của env sử dụng trong container
		 valueFrom:
			 configMapKeyRef:
				 name: tên của config map
				 key: key trong config map
```

Với các khai báo trực tiếp trong pods, có thể sử dụng configMap volume lấy data từ config map và sau đó sử dụng trong các container. Phù hợp cho trường hợp data cần thiết là các file chứa content dài.

Khai báo config map Volume
...
volumes:
	name: config 
	 configMap: 
	 name: fortune-config 
	 items: # trường hợp chỉ muốn lấy một số key từ ConfigMap
	 defaultMode: # chỉ định quyền cho tất cả các file trong volume 
```

Khi sử dụng volume, các file có trong folder đang mount với volume sẽ bị hidden đi vì VFS k còn mount với nó nữa. Thay vào đó các file trong volume sẽ hiện thị trong folder đó. Để không hidden các existing file, sử dụng subPath trong volumeMounts để mount cho file/sub dir cụ thể
...
Khi thay đổi data của ConfigMap, kubelet trên các node sẽ phát hiện thay đổi và đồng bộ dữ liệu mới xuống node.

Các volume đang tham chiếu tới ConfigMap sẽ được cập nhật (sau một khoảng thời gian ngắn, không phải realtime).

Tuy nhiên, để ứng dụng sử dụng config mới, bản thân ứng dụng cần có cơ chế reload (ví dụ: đọc lại file, watch file…). Nếu không có, sẽ xảy ra tình trạng:

- Pod mới → dùng config mới
- Pod cũ → vẫn dùng config cũ (đã load vào memory)

Khi Pod sử dụng ConfigMap volume, kubelet sẽ:

- lấy dữ liệu từ ConfigMap
- tạo các file trong thư mục local trên node
- mount vào container dưới dạng cấu trúc gồm file + symbolic link (`..data`) để hỗ trợ cập nhật atomically

Cụ thể:

```
file → ..data/file → versioned folder
```

Khi ConfigMap thay đổi:

- kubelet tạo thư mục version mới
- ghi dữ liệu mới
- cập nhật symlink `..data` trỏ sang version mới

→ container sẽ thấy nội dung file được cập nhật

---

Trong trường hợp sử dụng `subPath`:

- Kubernetes sẽ bind mount trực tiếp một file cụ thể vào container
- container không còn đi qua layer symlink (`..data`)

Khi ConfigMap update:

- kubelet tạo version folder mới
- nhưng container vẫn trỏ tới file cũ (inode cũ)

→ **subPath không hỗ trợ auto update**

### Secrets

Giống như ConfigMaps hỗ trợ lưu trữ các sensitive configuration nhờ các features hỗ trợ cho nó:

- Cho phép mã hoá khi lưu vào etcd, default chỉ encode base 64
- Secrets khi expose dưới dạng volume sẽ lưu vào tmpfs (memory) thay vì lưu xuống disk, giảm rủi ro bị leak data
